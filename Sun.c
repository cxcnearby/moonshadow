...
void sun_orbit( double mjd, double *SRAS, double *SDEC ){
  ...
  */
  ...
  de = 0.00256 * cos( ( 1934.0 * T + 235.0 ) * D2R ) + 0.00015 * cos( ( 72002.0 * T + 201.0 ) * D2R );
 ...
 *SDEC = SUMD;
 ...
 /*
  ...
  m = sin( SUMP * D2R ) * C * cos( TIBET_LA * D2R ) / cos( DEC );

  *SRAS = ( atan( m * sin( RAS - theta ) / ( 1 - m * cos( RAS - theta ) ) ) + RAS ) * R2D;

  if( *SRAS < 0.0) *SRAS += 360.0;
  if( *SRAS > 360.0) *SRAS -= 360.0;

  gamma = atan( S / C * tan( TIBET_LA * D2R ) * cos( ( *SRAS * D2R - RAS ) / 2.0 ) 
		/ cos( ( *SRAS * D2R + RAS ) / 2.0 - theta ) );
  n = sin( SUMP * D2R ) * S * sin( TIBET_LA * D2R ) / sin( gamma );

  *SDEC = ( atan( n * sin( DEC - gamma ) / ( 1 - n * cos( DEC - gamma ) ) ) + DEC ) * R2D;
  */
