...
double distance_equatorial(double ra1, double dec1, double ra2, double dec2)
  {
    return(acos(cos((ra2 - ra1)*deg_rad)*cos(dec1*deg_rad)*cos(dec2*deg_rad)+sin(dec1*deg_rad)*sin(dec2*deg_rad))*rad_deg);
  }
...
double distance_horizontal(double zen1, double azi1, double zen2, double azi2)
  {
    return(acos(cos((azi1 - azi2)*deg_rad)*sin(zen1*deg_rad)*sin(zen2*deg_rad)+cos(zen1*deg_rad)*cos(zen2*deg_rad))*rad_deg);
  }
