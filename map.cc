...
int main(int argc, char *argv[]) {
  ...
  TH2F *hOnS = new TH2F("hons", "on events (smoothed)", iMapBinNumber,
                        -(dMapRange + dBinWidth / 2.0),
                        dMapRange + dBinWidth / 2.0, iMapBinNumber,
                        -(dMapRange + dBinWidth / 2.0),
                        dMapRange + dBinWidth / 2.0);
  TH2F *hOffS = new TH2F("hoffs", "off events (smoothed)", iMapBinNumber,
                         -(dMapRange + dBinWidth / 2.0),
                         dMapRange + dBinWidth / 2.0, iMapBinNumber,
                         -(dMapRange + dBinWidth / 2.0),
                         dMapRange + dBinWidth / 2.0);
  TH2F *hExcS = new TH2F("hexcs", "on-off events (smoothed)", iMapBinNumber,
                         -(dMapRange + dBinWidth / 2.0),
                         dMapRange + dBinWidth / 2.0, iMapBinNumber,
                         -(dMapRange + dBinWidth / 2.0),
                         dMapRange + dBinWidth / 2.0);
  ...
  TH2F *hSigS2 = new TH2F("hsigs2", "2-D significance (smoothed)", iMapBinNumber,
                          -(dMapRange + dBinWidth / 2.0),
                          dMapRange + dBinWidth / 2.0, iMapBinNumber,
                          -(dMapRange + dBinWidth / 2.0),
                          dMapRange + dBinWidth / 2.0);
  ...
  for (Long64_t i = 0; i < nentries; i++) {
    cInput->GetEntry(i);
    if (s_EventSel.nfitc >= 100 && s_EventSel.compactness > 15 && s_EventSel.compactness < 500) {
      if (s_EventSel.id == 0) {
  ...
  for (int i = 1; i <= iBinNumber; i++) {
    for (int j = 1; j <= iBinNumber; j++) {
      ...
      hExc->SetBinContent(i, j, vOn[i][j] - vOff[i][j] / (2 * iWindowNumberOneSide));
    }
  }

  for (int i = 1; i <= iMapBinNumber; i++) {
    for (int j = 1; j <= iMapBinNumber; j++) {
      dSigSmooth = sig_smooth(vOn, vOff, i + iBinShift, j + iBinShift, iWindowNumberOneSide, dBinWidth,
                               iBinNumber, dSmoothRadius, &dNOn, &dNOff);
      ...
      if (vOn[i + iBinShift][j + iBinShift] != 0)
        dSig = (vOn[i + iBinShift][j + iBinShift] - vOff[i + iBinShift][j + iBinShift] / (2 * iWindowNumberOneSide)) /
               sqrt(vOn[i + iBinShift][j + iBinShift] + vOff[i + iBinShift][j + iBinShift] / (2 * iWindowNumberOneSide * 2 *
      ...
      if (j >= (iMapBinNumber + 1) / 2 - 30 && j <= (iMapBinNumber + 1) / 2 + 30)
        hRa->Fill((i - 1) * dBinWidth - dMapRange, vOn[i + iBinShift][j + iBinShift] - vOff[i + iBinShift][j + iBinShift] / (2 * iWindowNumberOneSide));
      if (i >= (iMapBinNumber + 1) / 2 - 30 && i <= (iMapBinNumber + 1) / 2 + 30)
        hDec->Fill((j - 1) * dBinWidth - dMapRange, vOn[i + iBinShift][j + iBinShift] - vOff[i + iBinShift][j + iBinShift] / (2 * iWindowNumberOneSide));
      if (i == (iMapBinNumber + 1) / 2 && j == (iMapBinNumber + 1) / 2)
        printf("center %f %f %f %f %f %f\n", dSmoothRadius, dSigSmooth, dNOn,
               dNOff, dTotalOn, dTotalOff);
    }
  }
  fMap->Write();
  fMap->Close();
